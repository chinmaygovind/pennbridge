...
public class Truss {
    ...
    public Truss(Joint leftAnchor, Joint rightAnchor, List<Joint> joints, List<Member> members) {
        ...
        TrussJoint trussLeftAnchor = new TrussJoint(leftAnchor);
        if (trussLeftAnchor.getY() != 0) {
            throw new IllegalArgumentException(
                    String.format("Left anchor %s must be at y-level 0!", leftAnchor)
            );
        }
        TrussJoint trussRightAnchor = new TrussJoint(rightAnchor);
        if (trussRightAnchor.getY() != 0) {
            throw new IllegalArgumentException(
                    String.format("Right anchor %s must be at y-level 0!", rightAnchor)
            );
        }
        this.joints = new ArrayList<>();
        for (Joint j : joints) {
            TrussJoint tj = new TrussJoint(j);
            if (j.equals(leftAnchor)) {
                this.leftAnchor = tj;
            }
            if (j.equals(rightAnchor)) {
                this.rightAnchor = tj;
            }
            this.joints.add(tj);
        }
    ...
    public Truss(
            TrussJoint leftAnchor, TrussJoint rightAnchor, List<TrussJoint> joints,
    ...
    public void applyLoad(Joint j, double load) {
        ...
        if (!joints.contains(tj)) {
            throw new IllegalArgumentException(
                    "Cannot apply load to joint " + j + " not in truss!"
            );
        }
        // assume load can only go vertically
        joints.get(joints.indexOf(tj)).setLoadForce(new Vec2D(0, load));
    }

    public void clearLoads() {
        for (TrussJoint tj : getJoints()) {
            tj.setLoadForce(Vec2D.ZERO);
        }
    }
    ...
    public double[][] getForceMatrix() {
        ...
        // right anchor (assumed roller) upward reaction force must balance out other
        // torques
        // recall F = torque / d
            ...
            if (Math.abs(firstTermToSubtract) < 0.001) {
                ...
                for (int newRow = rowToSubtract + 1; newRow < matrix.length; newRow++) {
                    if (Math.abs(matrix[newRow][rowToSubtract]) > 0.001) {
                        ...
                        matrix[rowToSubtract] = Arrays
                                .copyOf(matrix[newRow], matrix[newRow].length);
    ...
    public static final int SWING_ORIGIN_X = 100;
    public static final int SWING_ORIGIN_Y = 400;

    public static final int LEFT_ANCHOR_X = 0;
    public static final int RIGHT_ANCHOR_X = 20;
    public static final double TRUSS_COORDS_SCALE = 25;

    /**
     * The truss calculations use a different coordinate system.
     * The origin is at the left support joint of the bridge, with y increasing
     * upward and x increasing rightward.
    ...
    public static Vec2D toTrussCoords(Point2D p) {
        // x = 0 is at 100px, x = 20 is at 400px
        double newX = (p.getX() - SWING_ORIGIN_X) / TRUSS_COORDS_SCALE;
        // y = 0 is at 500px, flip upside down so it increases upward
        double newY = -(p.getY() - SWING_ORIGIN_Y) / TRUSS_COORDS_SCALE;
        return new Vec2D(newX, newY);
    }

    public static Vec2D toTrussCoords(double x, double y) {
        double newX = (x - 100) / 25.0; // x = 0 is at 100px, x = 20 is at 500px
        double newY = -(y - 500) / 25.0; // y = 0 is at 500px, flip upside down so it increases
                                         // upward
        return new Vec2D(newX, newY);
    }
}
